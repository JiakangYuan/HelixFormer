...
from utils.train_test import  train_test_helix_transformer_resnet12
...
def main(config):
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for data, _ in tqdm(train_loader, desc='train', leave=False):
            ...
            labels_query = fs.make_nk_label(n_train_way, n_train_query, ep_per_batch=ep_per_batch).cuda()
            labels_support = fs.make_nk_label(n_train_way, n_train_shot, ep_per_batch=ep_per_batch).cuda()

            acc, loss = train_test_helix_transformer_resnet12(embedding_net, relation_net, trans_net,
                                                              x_shot, x_query, labels_query, n_train_way, n_train_shot,
        ...
        for name, loader, name_l, name_a in [
            ...
            for data, _ in tqdm(loader, desc=name, leave=False):
                ...
                with torch.no_grad():
                    acc, loss = train_test_helix_transformer_resnet12(embedding_net, relation_net,
                                                                      trans_net, x_shot, x_query, labels_query, n_way,
                                                                      n_shot,  n_query, ep_per_batch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./configs/train_meta_helix_transformer.yaml')
