...
class Aircraft(Dataset):
    def __init__(self, root_path, split='train', **kwargs):
        ...
        split_tag = split
        txt_path = os.path.join(root_path, 'data', 'split', split_tag + '.csv')
        img_path = os.path.join(root_path, 'data', 'images')
        data, label = self.parse_csv(img_path, txt_path)
        image_size = 84
        data = [Image.open(x).convert('RGB').resize((image_size, image_size)) for x in data]
        min_label = min(label)
