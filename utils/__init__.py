...
def make_optimizer(params, name, lr, weight_decay=None, betas=(0.9, 0.999), milestones=None, stepsize=None, t_max=None, eta_mean=0):
    ...
    if name == 'sgd':
        ...
        optimizer = AdamW(params, lr, betas, weight_decay=weight_decay)
    if milestones:
        lr_scheduler = MultiStepLR(optimizer, milestones)
    elif stepsize:
        lr_scheduler = StepLR(optimizer, stepsize, gamma=0.5)
    elif t_max:
        lr_scheduler = CosineAnnealingLR(optimizer, t_max, eta_mean)
    else:
        lr_scheduler = None
