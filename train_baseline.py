...
def main(config):
    ...
    if save_name is None:
        save_name = 'Baseline_tiered_Meta_{}-{}shot'.format(
    ...
    if config.get('tval_dataset'):
        ...
        tval_sampler = CategoriesSampler(
            ...
            ep_per_batch=ep_per_batch)
        tval_loader = DataLoader(tval_dataset, batch_sampler=tval_sampler, num_workers=0, pin_memory=True)
    else:
        tval_loader = None
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for data, _ in tqdm(train_loader, desc='train', leave=False):
            ...
            acc, loss = train_relation_based(embedding_net, relation_net, x_shot, x_query,
                                             labels_query, n_train_way, n_train_shot,
                                             n_train_query, ep_per_batch, loss_type='softmax')
